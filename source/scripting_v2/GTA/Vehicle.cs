...
namespace GTA
{
	public sealed class Vehicle : Entity
	{
		...
		NumberPlateMounting NumberPlateMounting => (NumberPlateMounting)Function.Call<int>(Hash.GET_VEHICLE_PLATE_TYPE, Handle);

		NumberPlateType NumberPlateType => (NumberPlateType)Function.Call<int>(Hash.GET_VEHICLE_NUMBER_PLATE_TEXT_INDEX, Handle);
		...
		public VehicleColor PrimaryColor
		{
			...
			{
				int color1, color2;
				unsafe { Function.Call(Hash.GET_VEHICLE_COLOURS, Handle, &color1, &color2); }
		...
		public void PlaceOnNextStreet()
		{
			...
			for (int i = 1; i < 40; i++)
			{
				float heading;
				float val;
				unsafe { Function.Call(Hash.GET_NTH_CLOSEST_VEHICLE_NODE_WITH_HEADING, pos.X, pos.Y, pos.Z, i, outPos, &heading, &val, 1, 0x40400000, 0); }
