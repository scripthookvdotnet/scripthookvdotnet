...
namespace GTA
{
	Global::Global(int index) : mAddress(static_cast<unsigned long long *>(SHVDN::NativeMemory::GetGlobalPtr(index).ToPointer()))
	{
	}
	...
	{
		*(unsigned long long *)SHVDN::NativeMemory::GetGlobalPtr(index).ToPointer() = *value.mAddress;
	}
	...
	GameVersion Game::Version::get()
	{
		...
		{
			_gameVersion = static_cast<GameVersion>(SHVDN::NativeMemory::GetGameVersion() + 1);
		}
