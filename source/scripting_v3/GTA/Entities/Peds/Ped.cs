	...
	public sealed class Ped : Entity
	{
		...
		public bool DropsEquippedWeaponOnDeath
		{
			get
			{
				...
				return (SHVDN.NativeMemory.ReadByte(memoryAddress + offset) & (1 << 6)) == 0;
			}
		...
		public bool CanSufferCriticalHits
		{
			get
			{
				...
				return (SHVDN.NativeMemory.ReadByte(memoryAddress + offset) & (1 << 2)) == 0;
			}
		...
		{
			get => new GTA.RelationshipGroup(Function.Call<int>(Hash.GET_PED_RELATIONSHIP_GROUP_HASH, Handle));
